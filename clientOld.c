...
int main(int argc, char *argv[]) {
	...
	printf("Client key: %llu --> %s\n",intkey,key.arg1);	//key for the session
	...
	if (pthread_create(&send_thread, NULL, send_handler,(void*)&key) != 0) {
		perror("Failed to create send thread");
        	close(skfd);
		exit(1);
	}
	if (pthread_create(&recv_thread, NULL, receive_handler,(void*)&key) != 0) {
...
void *send_handler(void *arg) {
    ...
    while (1) {
        ...
        if (write(skfd, cipher, strlen(cipher)) < 0) break;
    }
...
unsigned long long dhexchange() {
	...
	const unsigned long long y2=(unsigned long long)(rand() % 90000000 + 10000000);
	unsigned long long base,mod,r1;
	...
	unsigned long long r2=modexp(base,y2,mod);
